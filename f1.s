main:
    ...
    addi    a2, zero, 0x1           # load a2 with 1 - a2 is the seed of the lfsr
...
iloop:
    ...
    addi    a0, a1, 0x0             # load a0 with a1
    bne     a1, t1, lfsr            # if a1 = 255, branch to lfsr
    bne     t1, zero, iloop         # else always branch to iloop

light:
    slli    a1, a1, 0x1             # left shift by 1
    addi    a1, a1, 0x1             # add 1
    addi    t3, zero, 0xa           # load t3 with 10
    j       delay                   # unconditional jump to delay
...
lfsr:
    ...
    j       countdown               # unconditional jump to countdown
